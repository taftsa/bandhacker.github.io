$(document).ready(function () {
    ...
    function addGraphCanvas() {
		...
		if (localStorage.getItem('help') == 'on') {
			$('#help').addClass('on');
		};
    };
	...
	if (localStorage.getItem('email') !== null || localStorage.getItem('band') !== null) {
		...
		document.getElementById('userBand').value = localStorage.getItem('band');
	};
	...
	$(document).on('click', '#rememberButton', function() {
		if ($(this).hasClass('forgetMe')) {
			...
			if (document.getElementById('userEmail').value.toString() !== "" || document.getElementById('userBand').value.toString() !== "") {
				$(this).addClass('forgetMe');				
				localStorage.setItem('email', document.getElementById('userEmail').value.toString());
				localStorage.setItem('band', document.getElementById('userBand').value.toString());
				document.getElementById('rememberButton').value = "Forget Me";
			} else {
	...
	//Functions handling filter buttons
    ...
    //Tooltip explaining challenge levels
    $(document).on({
        mouseenter: function (evt) {
            if (localStorage.getItem('help') == 'on') {
                if (this.id[2] == "1") {
        ...
        openOverlay('https://docs.google.com/forms/d/e/1FAIpQLSf7CKt6BjXfYagKP9XWO74g6PdyYAiAoWhEcvCfUinXGbpcDA/viewform', true, function () {
            $('iframe#pageContent').load(function () {
                ...
                function checkUntilLoaded() {
					...
					//This checks to see if a new band has been created
                    if (listOfUsers.length > numberOfUsers) {
						//newBandCreated makes sure that, if an extra Timeout event is fired, the function doesn't run twice
                        if (!newBandCreated) {
                            newBandCreated = true;
                            closeOverlay();
    ...
    $(document).on('click', '#addPiece', function () {
		openOverlay('https://docs.google.com/forms/d/e/1FAIpQLSe5q7jbSlC4H0u8NPEeAaniK09N4vqj9ZoStJSBG3R4SQsgUQ/viewform', true, function () {
            $('iframe#pageContent').load(function () {
                $('#overlayPane').empty();
                $('#overlayPane').prepend('<div class="loaderExternal"></div>');
	...
	$(document).on('click', '#filterBox', function() {
		if (!animating && piecesLoaded && !viewingFilters) {
				...
				$(this).animate({ height: '120px' }, 300, function() {
					animating = false;					
				
					//State filter
					$(this).append('<select class="filter" id="stateFilter" name="stateFilter"><option value="Any State">Any State</option></select>');
					...
					//Grade filter
					$(this).append('<select class="filter" id="gradeFilter" name="gradeFilter"><option value="Any Grade">Any Grade</option></select>');
					//Looks through all pieces
					for (var f = 0; f < activePieceList.length; f++) {
					...
					$(this).append('<select class="filter" id="classificationFilter" name="classificationFilter"><option value="Any Classification">Any Classification</option></select>');
					//Looks through all pieces
					for (var f = 0; f < activePieceList.length; f++) {
	...
	$(document).on('click', '#applyFilters', function(){
		...
		if (stateValue !== 'Any State') {			
			for (var sf = 0; sf < $('.piece').length; sf++) {				
				var thisPieceStates = activePieceList[$('.piece:eq(' + sf + ')').attr('id') / 1]["Reference States"];
		...
		if (gradeValue !== 'Any Grade') {			
			for (var sf = 0; sf < $('.piece').length; sf++) {				
				var thisPieceGrades = activePieceList[$('.piece:eq(' + sf + ')').attr('id') / 1]["Grades"];
		...
		if (classificationValue !== 'Any Classification') {			
			for (var sf = 0; sf < $('.piece').length; sf++) {				
				var thisPieceClassifications = activePieceList[$('.piece:eq(' + sf + ')').attr('id') / 1]["Classifications"];
				for (var sfa = 0; sfa < thisPieceClassifications.length; sfa++) {
					if (thisPieceClassifications[sfa][0] == classificationValue) {
						allow.classification[sf] = true;
					};
				};				
			};
		} else {
		...
		//Apply selected filters (add more filters to the if condition using &&)
		for (var af = 0; af < $('.piece').length; af++) {
			if (!(allow.state[af] && allow.grade[af] && allow.classification[af])) {
				$('.piece:eq(' + af + ')').css('display', 'none');
			};
		};
	});
});